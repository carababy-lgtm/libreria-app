...
def lookup_espanso(autore, titolo):
    """
    Tentativi in ordine:
      1. Google Books: autore + titolo
      2. Google Books: titolo completo
      3. Google Books: prime 4 parole titolo
      4. Open Library: autore + titolo
      5. Open Library: titolo completo
    Se descrizione mancante, prova la fonte alternativa.
    """
    breve = _prime_parole(titolo, 4)
    tentativi = [
        (_google,      f"{autore} {titolo}".strip(), "Google(autore+titolo)"),
        (_google,      titolo,                        "Google(titolo)"),
        (_google,      breve,                         "Google(titolo breve)"),
        (_openlibrary, f"{autore} {titolo}".strip(), "OpenLib(autore+titolo)"),
        (_openlibrary, titolo,                        "OpenLib(titolo)"),
    ]
...
# ── LOOKUP SINGOLO (per inserimento manuale) ───────────────
def lookup_singolo(query):
    """Ricerca veloce per un singolo libro (usata dal form manuale)."""
    r = _google(query)
    if r:
